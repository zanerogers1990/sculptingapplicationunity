...
namespace Sculpting
{
    ...
    ///
    /// Unlike Join this is NOT destructive of the other objects: they are only hidden (or, if
    /// the user asks, deleted). The second object is usually worth keeping - the same cutter is
    /// what makes the matching half of a two-part mold, and any boolean you want to redo at a
    /// different resolution needs its operands back.
    public static class MeshBooleanTool
    {
        /// Applies `op` between `target` and `others`, replacing the target's mesh with the
                                 ...
                                 bool hideOthers, bool deleteOthers, out string message)
        {
            ...
            int affected = 0;
            foreach (SculptableMesh other in others)
            {
                if (other == null || other == target) continue;
                affected++;
                if (deleteOthers) selection?.DeleteObject(other);
                else if (hideOthers && other.Visible) selection?.SetVisible(other, false);
            }

            // Leaves only the target selected: the others are gone or hidden, and a selection
